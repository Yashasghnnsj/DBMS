    ...
    if (loading) {
        return (
            <div className="flex flex-col justify-center items-center h-[80vh] gap-4">
                ...
                <p className="text-slate-400 font-black uppercase tracking-widest text-[10px]">Setting up your dashboard...</p>
            </div>
        );
    }
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                <div>
                    <div className="flex items-center gap-3 mb-4">
                        <div className="px-3 py-1 bg-slate-900 rounded-full text-[10px] font-black text-white uppercase tracking-widest">Study Progress: Active</div>
                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Level 12 Student</div>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                    <div className="flex items-center gap-4 bg-white/80 backdrop-blur-xl px-6 py-4 rounded-[2rem] border border-white/50 shadow-soft">
                        <div className="text-right">
                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Learning Speed</div>
                ...
                <div className="lg:col-span-8 space-y-10">
                    ...
                    <div className="glass-card p-10 relative overflow-hidden">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                <h3 className="text-2xl font-black text-slate-900 tracking-tight">Study Heatmap</h3>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Study habits over 30 days</p>
                            </div>
                    ...
                    {/* Performance Trends */}
                    <div className="glass-card p-10 bg-slate-900 hover:bg-slate-900 text-white border-none relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-10 opacity-10">
                            <BarChart3 size={120} />
                        </div>
                        <h3 className="text-2xl font-black tracking-tight mb-8 text-white">Concept Mastery Trends</h3>
                        <div className="space-y-8 relative z-10">
                            {detailedStats?.performance_trend?.length > 0 ? detailedStats.performance_trend.map((p, i) => (
                                <div key={i} className="space-y-3">
                                    <div className="flex justify-between items-end">
                                        <div className="flex items-center gap-3">
                                            <div className="text-xs font-black uppercase tracking-widest text-slate-400">{p.date}</div>
                                            <div className="text-sm font-bold text-slate-100 truncate max-w-[200px]">{p.quiz}</div>
                                        </div>
                        ...
                        <div className="space-y-4">
                            <button onClick={() => window.location.href = '/courses'} className="w-full flex items-center justify-between p-5 rounded-[1.5rem] bg-white/10 hover:bg-white/20 transition-all border border-white/10 group">
                                <div className="flex items-center gap-4">
                                    <BookOpen size={20} />
                                    <span className="font-bold uppercase tracking-widest text-[10px]">My Courses</span>
                                </div>
                            ...
                            <button onClick={() => window.location.href = '/learning-path'} className="w-full flex items-center justify-between p-5 rounded-[1.5rem] bg-slate-900 border border-white/5 hover:-translate-y-1 transition-all group shadow-premium">
                                <div className="flex items-center gap-4">
                                    ...
                                    <span className="font-bold uppercase tracking-widest text-[10px]">Smart Update</span>
                                </div>
